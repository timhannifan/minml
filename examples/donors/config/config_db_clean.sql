...
create table cleaned.projects as (
    with cleaned as (
        select
            ...
            btrim(lower(school_metro)) as metro,
            btrim(replace(regexp_replace(btrim(lower(school_district)), '\s{2,}|,|\.',''), $$'$$,'')) as district,
            ...
            btrim(replace(regexp_replace(btrim(lower(teacher_prefix)), '\s{2,}|,|\.',''), $$'$$,'')) as teacher_prefix,
            case
